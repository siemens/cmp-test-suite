...
def cast_csr_to_asn1(data: csr_type) -> rfc2986.CertificationRequest:
    """
    ...
    if isinstance(data, rfc2986.CertificationRequest):
        ...
        return cast_csr_to_asn1(data.public_bytes(serialization.Encoding.DER))
    else:
        # Raise an error if the input type is unsupported.
        raise ValueError(f'Unsupported type: {type(data)}')


def cast_asn1csr_to_csr(data: Union[bytes, x509.CertificateSigningRequest, rfc2986.CertificationRequest]) -> x509.CertificateSigningRequest:
    """
