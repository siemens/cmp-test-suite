...
def verify_signature(cert: Union[x509.Certificate, x509.CertificateSigningRequest], public_key: ASYM_PUBLIC_KEY = None):
    """
...
def verify_cert_signature(cert: CERT_TYPE, public_key: ASYM_PUBLIC_KEY = None):
    """
...
def verify_csr_signature(csr: CSR_TYPE, public_key: ASYM_PUBLIC_KEY = None):
    """
    Verifies the digital signature of an X.509 CSR using the provided or extracted public key.


    :param csr:
        The certificate to verify, represented in a type that can be cast to an X.509 CSR object.
        It should be in a DER-encoded form compatible with the `x509.Certificate` type.
    :param public_key:
        The public key used for verifying the certificate's signature.
        This parameter is optional. If not provided, the public key is extracted from the certificate.
