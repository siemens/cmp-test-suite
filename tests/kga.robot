...
Test Tags           kga
Suite Setup    Set Up EnvelopedData Certs For Tests
...
CA MUST Accept A Valid KGA Request For ML-DSA
    ...
    [Tags]            ir    positive  ml-dsa   pq  pq-sig
    ...
    ${private_key}=   Validate Not Local Key Gen    ${response}   expected_type=ktri   ee_key=${KGA_KTRI_KEY}
...
CA MUST Accept A Valid KGA Request For ML-KEM
    ...
    [Tags]            positive  ml-kem  pq
    ${is_set}=    Is Certificate And Key Set    ${KGA_KTRI_CERT}    ${KGA_KTRI_KEY}
    Skip If    not ${is_set}    Skipped because KGA_KTRI_CERT and KGA_KTRI_KEY are not set.
    ${key}=   Generate Key    ${DEFAULT_ML_KEM_ALG}
    ${cm}=    Get Next Common Name
    ${ir}=    Build Ir From Key    ${key}   ${cm}   for_kga=True   sender=${SENDER}
    ...       exclude_fields=senderKID,sender
    ${protected_ir}=    Protect PKIMessage
    ...    pki_message=${ir}
    ...    protection=signature
