...
class TestMLDSAKGASigner(unittest.TestCase):
    ...
    def _generate_kga_extensions(self, key: SLHDSAPrivateKey) -> rfc9480.Extensions:
    ...
    def _generate_kga_cert(self, key: SLHDSAPrivateKey) -> rfc9480.CMPCertificate:
        ...
        cert, _ = build_certificate(
            ...
            ca_key=self.ca_key,
            serial_number=1,
            common_name="CN=SLH-DSA KGA Test Key",
        )
        return cert
    ...
    def test_slh_dsa_sha2_256s_kga_signer(self):
        """
        GIVEN a KGA request IR.
        WHEN the KGA CMP response is built, with a SLH-DSA-SHA2-256s key.
        THEN it should validate that the key is not a local key generation key.
        """
        kga_key = generate_key("slh-dsa-sha2-256s")
    ...
    def test_slh_dsa_sha2_128s_kga_signer(self):
        """
        GIVEN a KGA request IR.
        WHEN the KGA CMP response is built, signed with a SLH-DSA-SHA2-128s key.
        THEN it should validate that the key is not a local key generation key.
        """
        kga_key = generate_key("slh-dsa-sha2-128s")
        ir = self._generate_kga_request()
        response, issued_certs = self._build_kga_cmp_response(ir, kga_key)
        self.assertIsInstance(response, PKIMessageTMP)
        validate_not_local_key_gen(response, password=self.pre_shared_secret, expected_type="pwri")
