...
def _load_composite_sig07_from_public_bytes(algorithm: str, public_key: bytes) -> CompositeSig07PublicKey:
...
class TestVectorEntry:
    ...
    def validate(self):
        ...
        public_key = _load_composite_sig07_from_public_bytes(self.name, public_key_bytes)
...
class TestCompositeSig07TestVectors(unittest.TestCase):
    ...
    def test_composite_sig07_cert_from_vectors_rsa(self):
        """
        ...
        for vector in self.test_cases:
            ...
            with self.subTest(vector=vector["tcId"]):
                ...
                public_key = _load_composite_sig07_from_public_bytes(test_vec.name, spki["subjectPublicKey"].asOctets())
                if public_key != private_key.public_key():
                    raise ValueError("Public key does not match the private key's public key.")

                alg_id = cert["tbsCertificate"]["signature"]

                _name = COMPOSITE_SIG07_OID_TO_NAME[alg_id["algorithm"]]
                use_pss = False
                if "pss" in _name:
                    use_pss = True
    ...
    def test_composite_sig07_cert_from_vectors_ed(self):
        """
    ...
    def test_composite_sig07_cert_from_vectors_ecdsa(self):
        """
