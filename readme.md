...
prerequisites  
- Linux or WSL (for Windows, also ensure to use the Linux filesystem).
- Docker
- [Compose compose v2](https://github.com/docker/compose?tab=readme-ov-file)

To use EJBCA docker, set `--variable environment` to `ejbca`.
...
- Wherever `univ.<ClassName>` appears in the code commentary (e.g., `univ.ObjectIdentifier`), it is assumed to refer to the `univ` module from the `pyasn1` library. 
The actual import statement is `from pyasn1.type import univ`.

- Wherever `rfc<num>.<ClassName>` appears in the code commentary (e.g., `rfc9480.PKIMessage`), it is assumed to be from the `pyasn1_alt_modules` library.
The Actual import is `from pyasn1_alt_module import rfc9480`.


## Imports

When importing modules, always use top-level imports for function imports. Otherwise, 
the documentation generated by the Robot Framework will include the imported functions 
within the module that imports them.

In other words, use `import cryptoutils`, then call `cryptoutils.sign_data` in your code, rather than `from cryptoutils import sign_data`.
...
## String parameters
We pass lists of strings into functions as a single comma-separated string, e.g., `Asn1 Must Contain Fields    ${pki_header}    pvno,sender,recipient`. This is syntactic sugar for invocation from within RobotFramework tests, so there is no need to first create a list and populate it with items - thus the code is more concise.

## Mask internal-use Python functions
If you don't want a Python function to be exposed as a keyword in Robot Framework, use the `@not_keyword` decorator.
...
# Development
On a development system you can install additional dependencies to automate some code quality and formatting checks.

1. Install the dependencies: `pip install -r requirements-dev.txt`
2. If you use IntelliJ or PyCharm, change the working directory via `File -> Project Structure... -> Modules`, click on the resources folder and click on `Source` and apply.
